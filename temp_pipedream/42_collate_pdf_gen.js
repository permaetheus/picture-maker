...
export default defineComponent({
  ...
  async run({steps, $}) {
    ...
    try {
      ...
      const bookQuery = `books?id=eq.${bookId}&select=recipient_id`
      ...
      const recipientId = bookData[0].recipient_id
      if (!recipientId) {
        throw new Error(`No recipient found for book ID ${bookId}`)
      }

      const recipientQuery = `recipients?id=eq.${recipientId}&select=name,message`
      const recipientData = await axios($, {
        url: `${baseURL}/${recipientQuery}`,
        headers
      })
      ...
      const { name, message } = recipientData[0]
      if (!name) {
      ...
      // Get portraits with style information and hires image keys
      console.log("Fetching portrait data...")
      const portraitsQuery = `portraits?book_id=eq.${bookId}&select=hires_image_key,style_id,artist_styles(page)&order=created_at.asc`
      ...
      const images = portraitData
        .filter(portrait => {
          ...
          console.log("Processing portrait:", {
            hasImage: !!portrait.hires_image_key,
            styleId: portrait.style_id,
            page: portrait.artist_styles?.page
          })

          // Validate each portrait has required data
          if (!portrait.hires_image_key) {
            console.warn(`Portrait missing hires_image_key for book ${bookId}`)
            return false
          }
        ...
        .map(portrait => ({
          imageUrl: portrait.hires_image_key,
          styleId: portrait.style_id,
          page: portrait.artist_styles.page
        }))
